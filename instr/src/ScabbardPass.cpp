...
protected:
  ...
  struct scabbardRTL {
    Function* MemoryAccess = nullptr;
    FunctionCallee MemoryAccess_callee;
    Function* SyncEvent = nullptr;
    FunctionCallee SyncEvent_callee;
    Function* StaticSharedAlloc = nullptr;
    FunctionCallee StaticSharedAlloc_callee;
    Function* RegisterMetadata = nullptr;
    FunctionCallee RegisterMetadata_callee;
    MetadataHandler Metadata;
  } scabbard;

public:
  scabbardIDevicePass() = delete;
  scabbardIDevicePass(const Module& M_) : _M(M_) { initFromModule(M_); }
...
protected:
  ...
  virtual void registerRTL(const Module& M) final {
    // TODO register the items defined in the RTL
  }

  virtual void initFromModule(const Module& M) {
    ...
    if (isa<AllocaInst>(Obj)) {
      ...
      if (auto* Global = dyn_cast<GlobalVariable>(Load->getPointerOperand())) {
        if (Global->getName().ends_with(".device") 
            || _M.getGlobalVariable(Global->getName()+".device"))
          ObjPO = GLOBAL;
        else if (Global->getName().ends_with(".managed") 
            || _M.getGlobalVariable(Global->getName()+".managed"))
          ObjPO = SHARED;
      }
